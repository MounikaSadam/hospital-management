...
<table BORDER=3><br><br><br>
	<tr>
		<td>DEPARTMENT ID<input type="text" name="t1" maxlength=10 size=20><BR>
		</td>
...
{	a=request.getParameter("Button1"); 
	...
	if(str1!="")
	{	
		String sql2="select * from department1 where dept_id='"+str1+"'";
...
try{
	...
	Connection c=DriverManager.getConnection ("jdbc:mysql://localhost:3306/hospital","root","");
	Statement s= c.createStatement();
	if(a.equals("ADD DEPARTMENT"))
	{	

	if(str1!=""&&str2!=""&&str3!=""&&str4!=""&&str5!="")
	{	
		String sql="insert into department1 values("+str1+",'"+str2+"','"+str3+"','"+str4+"',"+str5+")";
		out.println(sql);
		s.executeUpdate(sql);
	...
	else if(a.equals("DELETE DEPARTMENT PROFILE"))
	{
		if(str1!="")
		{
			String sq="select * from department1 where dept_id="+str1+"";
			ResultSet rs1=s.executeQuery(sq);
			if(rs1.next())
			{	
				
				String sql="delete from department1 where dept_id="+str1+"";
				s.execute(sql);
				%><script language="javascript">
	...
	else if(a.equals("EDIT DEPARTMENT PROFILE"))
	{
		if(str1!="")
		{
			...
			{	
				String sql="delete from department1 where dept_id="+str1+"";
				s.execute(sql);
				String sql1="insert into department1 values("+str1+",'"+str2+"','"+str3+"','"+str4+"',"+str5+")";
				s.execute(sql1);
				%><script language="javascript">
...
{	
	...
	Connection c=DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital","root","");
	Statement s= c.createStatement();
	String q="select * from department1";
